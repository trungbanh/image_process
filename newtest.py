...
class Ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # self.widget_1.setGeometry(0,0,400,600)
        self.show()
    ...
    def blurer(self,typer, path ):
        ...
        if (POSITION['start'] == (0,0) or POSITION['end'] == (0,0)):
            ...
            if (typer == 0) :
                ...
                imgB[int (POSITION['start'][1]):int (POSITION['end'][1]),:] = imgR
            if (typer == 1) :
                imgR = img[:,int(POSITION['start'][0]):int(POSITION['end'][0])]
                imgB = cv2.GaussianBlur(imgB,(self.kernel,self.kernel),0)
                imgB[:,int(POSITION['start'][0]):int(POSITION['end'][0])] = imgR

                print ("B: ",imgB[:,int(POSITION['start'][0]):int(POSITION['end'][0])].shape," R: ",imgR.shape)
